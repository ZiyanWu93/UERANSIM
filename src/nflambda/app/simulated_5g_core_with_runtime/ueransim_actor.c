...
void ueransim_startup_event_source(void)
{
    ...
    if (!started) {
        ...
        if (time(NULL) - start_time >= 1) {
            ...
            static uint8_t reg_request[] = {0x7e, 0x00, 0x41, 0x79, 0x00, 0x0d, 0x01, 0x99, 0xf9, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x2e, 0x04, 0x80, 0xf0, 0x80, 0xf0};
            size_t reg_request_len = sizeof(reg_request);
            
            printf("[UERANSIM] Initiating registration with Registration Request\n");
            print_received_pdu("Sending NAS PDU", reg_request, reg_request_len);
            ue_state = UE_STATE_REGISTERING;
            
            trigger_event(EVENT_TO_AMF, reg_request, reg_request_len);
            started = true;
        }
    }
}
